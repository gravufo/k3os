...
Download the ISO from the latest [release](https://github.com/gravufo/k3os/releases) and run it
...
| k3os.install.iso_url    |         | https://github.com/gravufo/k3os/../k3os-amd64.iso | ISO to download and install from if booting from kernel/vmlinuz and not ISO. |
...
You can install k3OS to a block device from any modern Linux distribution. Just download and run [install.sh](https://raw.githubusercontent.com/gravufo/k3os/master/install.sh).
...
Example: ./install.sh /dev/vda https://github.com/gravufo/k3os/releases/download/v0.10.0/k3os.iso
...
A special mode of installation is designed to install to a current running Linux system. This only works on ARM64 and x86_64. Download [install.sh](https://raw.githubusercontent.com/gravufo/k3os/master/install.sh)
...
```bash
./install.sh --takeover --debug --tty ttyS0 --config /tmp/config.yaml --no-format /dev/vda1 https://github.com/gravufo/k3os/releases/download/v0.10.0/k3os.iso
```
...
```bash
curl -sfL https://github.com/gravufo/k3os/releases/download/v0.10.0/k3os-rootfs-arm.tar.gz | tar zxvf - --strip-components=1 -C /
...
Integration with [rancher/system-upgrade-controller](https://github.com/rancher/system-upgrade-controller) has been implemented as of [v0.9.0](https://github.com/gravufo/k3os/releases/tag/v0.9.0).
To enable a k3OS node to automatically upgrade from the [latest GitHub release](https://github.com/gravufo/k3os/releases/latest) you will need to make sure it has the label
...
kubectl apply -f https://raw.githubusercontent.com/gravufo/k3os/v0.10.0/overlay/share/rancher/k3s/server/manifests/system-upgrade-controller.yaml
# after the system-upgrade-controller pod is Ready, apply the plan manifest (once per cluster)
kubectl apply -f https://raw.githubusercontent.com/gravufo/k3os/v0.10.0/overlay/share/rancher/k3s/server/manifests/system-upgrade-plans/k3os-latest.yaml
...
The source for the kernel is in `https://github.com/gravufo/k3os-kernel` and similarly you
